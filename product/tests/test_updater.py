...
def test_update_replaces_program_but_preserves_all_owner_data(tmp_path: Path) -> None:
    ...
    assert (install / "product" / "app" / "program.txt").read_text() == "new program"
    assert (
        install / "codex-plugin-marketplace" / "plugins" / "plugin.txt"
    ).read_text() == "new plugin"
    assert (install / "product" / ".env").read_text() == "OPENAI_API_KEY=local-secret\n"
    workspace = install / "product" / "投研数字员工"
    assert (workspace / "00_老板投资说明书.md").read_text() == "只看 A 股和港股"
    assert (workspace / "conversations" / "old.md").read_text() == "老板历史对话"
    assert (workspace / "reports" / "old.md").read_text() == "老板历史报告"
...
def test_failed_health_can_roll_back_program_and_database(tmp_path: Path) -> None:
    ...
    assert (install / "product" / "app" / "program.txt").read_text() == "old program"
    assert (
        install / "codex-plugin-marketplace" / "plugins" / "plugin.txt"
